...
def propensity_score_matching(df, treatment_col, outcome_col, covariate_cols):
    ...
    return {
        ...
        'balance_metrics': calculate_balance_metrics(df, covariate_cols)
    }

def calculate_balance_metrics(df, covariate_cols):
    """Calcula métricas de balance después del PSM"""
    treated = df[df['treatment'] == 1][covariate_cols]
    control = df[df['treatment'] == 0][covariate_cols]
...
def calculate_arima_forecast(data, periods=30, order=(1,1,1)):
    ...
    ARIMA, _, _, _, _, _ = get_heavy_imports()
    try:
...
def generate_asis_report(data_dict, format='pdf'):
    ...
    if 'vigilance' in data_dict:
        ...
        story.append(Paragraph(f"<b>Casos Totales:</b> {v.get('total_cases', 'N/A'):,}", styles['Normal']))
        story.append(Paragraph(f"<b>R0 Estimado:</b> {v.get('r0', 'N/A')}", styles['Normal']))
        story.append(Paragraph(f"<b>IFR (Tasa Fatalidad):</b> {v.get('ifr', 'N/A'):.2f}%", styles['Normal']))
        story.append(Paragraph(f"<b>Recuperados:</b> {v.get('recovered', 'N/A'):,}", styles['Normal']))
        story.append(Paragraph(f"<b>Fallecidos:</b> {v.get('deaths', 'N/A'):,}", styles['Normal']))
        story.append(Spacer(1, 12))
...
if not st.session_state.get("auth", False):
        ...
        if st.session_state.df_master is not None:
            ...
            **Leyenda de Estado:**
            ...
            with st.expander("👁️ Vista Previa de Datos Filtrados", expanded=True):
                st.dataframe(df_filtered.head(100) if 'df_filtered' in dir() else df.head(100),
    ...
    # ==========================================
        ...
        study_type = st.selectbox("🎯 Tipo de Estudio:",
            ["📊 Cohortes", "🔬 Casos y Controles", "🧪 Ensayos Clínicos", "🔍 Observacional", "📈 Multivariada", "⚙️ Confundidores"])
    ...
    # ==========================================
        ...
        with col_metrics[3]:
            st.metric("Recuperados", f"{df_v['Recuperados'].iloc[-1]:,}")
        with col_metrics[4]:
            st.metric("Fallecidos", f"{df_v['Fallecidos'].sum():,}")
        ...
        if len(df_v) > 3:
            with st.spinner("⏳ Ejecutando modelo..."):
                ...
                I0 = casos_reales // 10
                E0 = I0 * 2
                R0_init = int(df_v['Recuperados'].iloc[-1])
                S0 = N - E0 - I0 - R0_init
                ...
                if model_type == "SEIR + Monte Carlo":
                    ...
                    std_activos = max(df_v['Activos'].std(), 1)
                    std_rec = max(df_v['Recuperados'].std(), 1)
                    std_fall = max(df_v['Fallecidos'].std(), 1)

                    for _ in range(n_sim):
                        noise_n = np.random.normal(0, std_err * 0.6, size=base_preds.shape)
                        all_sims_nuevos.append(np.maximum(0, base_preds + noise_n))
                        all_sims_activos.append(np.maximum(0, df_v['Activos'].iloc[-1] + np.random.normal(0, std_activos, size=base_preds.shape)))
                        all_sims_recuperados.append(np.maximum(0, df_v['Recuperados'].iloc[-1] + np.cumsum(np.maximum(0, base_preds + noise_n) * 0.7)))
                        all_sims_fallecidos.append(np.maximum(0, df_v['Fallecidos'].iloc[-1] + np.cumsum(np.maximum(0, base_preds + noise_n) * 0.02)))
                    ...
                    futuro_dias = list(range(len(df_v) + 1, len(df_v) + 16))

                    fig_proj = go.Figure()
                    fig_proj.add_trace(go.Scatter(
                        x=list(df_v['Dia']) + futuro_dias,
                        y=list(df_v['Nuevos']) + p_nuevos_mean.tolist(),
                        name="Nuevos",
                    ...
                    fig_proj.add_trace(go.Scatter(
                        x=list(df_v['Dia']) + futuro_dias,
                        y=list(df_v['Activos']) + p_activos_mean.tolist() if len(df_v) < len(futuro_dias) else list(df_v['Activos']) + p_activos_mean.tolist(),
                        name="Activos",
                        line=dict(color='#ef4444', width=2)
                    ))
                    fig_proj.add_trace(go.Scatter(
                        x=list(df_v['Dia']) + futuro_dias,
                        y=list(df_v['Recuperados']) + p_rec_mean.tolist(),
                        name="Recuperados",
                        line=dict(color='#10b981', width=2)
                    ))
                    fig_proj.add_trace(go.Scatter(
                        x=list(df_v['Dia']) + futuro_dias,
                        y=list(df_v['Fallecidos']) + p_fall_mean.tolist(),
                        name="Fallecidos",
                    ...
                    arima_order = st.multiselect("Orden ARIMA (p,d,q):", [0, 1, 2], default=[1, 1, 1], format_func=lambda x: f"p={x}" if x == 0 else f"p={x}" if x == 0 else str(x))

                    if st.button("🔮 EJECUTAR ARIMA", use_container_width=True):
                        try:
                            arima_result = calculate_arima_forecast(df_v['Nuevos'].values, periods=15, order=(1, 1, 1))
                            if 'error' not in arima_result:
                                futuro_dias = list(range(len(df_v) + 1, len(df_v) + 16))
                                fig_arima = go.Figure()
                                fig_arima.add_trace(go.Scatter(x=df_v['Dia'], y=df_v['Nuevos'], name="Histórico", line=dict(width=3)))
                                ...
                                fig_arima.update_layout(title="🔮 ARIMA Forecast", height=400, template="plotly_dark")
    ...
    # ==========================================
        ...
        with tabs[0]:
            ...
            if uploaded:
                ...
                persist_user_data()
                st.success(f"✅ {len(st.session_state.survival_data)} registros!")
