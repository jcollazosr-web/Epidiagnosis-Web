...
if st.session_state.auth and 'user' in st.session_state and not st.session_state.data_loaded:
    ...
    if user_data:
        try:
            ...
            # Cargar df_v (vigilancia) — normalizar tipos al restaurar desde JSON
            if 'df_v' in user_data and user_data['df_v'] is not None:
                _df_v_loaded = pd.DataFrame(user_data['df_v'])
                if 'Fecha' in _df_v_loaded.columns:
                    _df_v_loaded['Fecha'] = pd.to_datetime(_df_v_loaded['Fecha'], errors='coerce').dt.date
                for _c in ['Dia', 'Activos', 'Nuevos', 'Recuperados', 'Fallecidos']:
                    if _c in _df_v_loaded.columns:
                        _df_v_loaded[_c] = pd.to_numeric(_df_v_loaded[_c], errors='coerce').fillna(0).astype(int)
                st.session_state.df_v = _df_v_loaded
...
if not st.session_state.get("auth", False):
    ...
    with st.sidebar:
        ...
        # Botón para borrar bases de datos cargadas
        if st.button("🗑️ BORRAR BASES DE DATOS", use_container_width=True):
            st.session_state['_confirm_clear_db'] = True

        if st.session_state.get('_confirm_clear_db', False):
            st.warning("⚠️ ¿Seguro que desea borrar TODAS las bases de datos cargadas? Esta acción no se puede deshacer.")
            col_confirm = st.columns(2)
            with col_confirm[0]:
                if st.button("✅ Sí, borrar todo", use_container_width=True, key="confirm_clear_yes"):
                    # Limpiar todas las bases de datos en session_state
                    dbs_to_clear = [
                        'df_master', 'df_v', 'df_v_edited',
                        'meta_studies', 'survival_data', 'forest_studies',
                        'map_data', 'marker_data', 'roc_data',
                        'articulos_pico', 'prisma_data', 'rob_assessments',
                        'psm_results', 'vigilance_config', 'data_loaded',
                        'modelo_ejecutado'
                    ]
                    defaults = {
                        'df_master': None, 'df_v': None, 'df_v_edited': None,
                        'meta_studies': pd.DataFrame(), 'survival_data': None,
                        'forest_studies': pd.DataFrame(), 'map_data': None,
                        'marker_data': None, 'roc_data': None,
                        'articulos_pico': [], 'prisma_data': {},
                        'rob_assessments': [], 'psm_results': None,
                        'vigilance_config': {}, 'data_loaded': False,
                        'modelo_ejecutado': False
                    }
                    for key in dbs_to_clear:
                        st.session_state[key] = defaults.get(key, None)
                    # Borrar también del archivo persistente del usuario
                    if st.session_state.get('auth') and st.session_state.get('user'):
                        save_user_data(st.session_state.user, {'last_save': datetime.now().isoformat()})
                    st.session_state['_confirm_clear_db'] = False
                    st.success("✅ Todas las bases de datos han sido borradas.")
                    st.rerun()
            with col_confirm[1]:
                if st.button("❌ Cancelar", use_container_width=True, key="confirm_clear_no"):
                    st.session_state['_confirm_clear_db'] = False
                    st.rerun()
    ...
    # ==========================================
        ...
        with st.expander("📥 Datos del Brote (CON FECHAS)", expanded=True):
            ...
            # Normalizar tipos ANTES del data_editor para evitar StreamlitAPIException
            # en _check_type_compatibilities (Fecha como str desde JSON no es compatible con DateColumn)
            if st.session_state.df_v_edited is not None:
                _df_edit = st.session_state.df_v_edited.copy()
                # Convertir Fecha a datetime.date (DateColumn requiere date, no str ni Timestamp)
                if 'Fecha' in _df_edit.columns:
                    _df_edit['Fecha'] = pd.to_datetime(_df_edit['Fecha'], errors='coerce').dt.date
                # Convertir columnas numéricas a int
                for _col in ['Dia', 'Activos', 'Nuevos', 'Recuperados', 'Fallecidos']:
                    if _col in _df_edit.columns:
                        _df_edit[_col] = pd.to_numeric(_df_edit[_col], errors='coerce').fillna(0).astype(int)
                st.session_state.df_v_edited = _df_edit
