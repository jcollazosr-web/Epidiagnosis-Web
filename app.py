...
if not st.session_state.get("auth", False):
    ...
    # ==========================================
        ...
        with st.expander("📥 Datos del Brote (CON FECHAS)", expanded=True):
            ...
            # Editor de datos con fechas - Formulario para control de edición
            with st.form("vigilancia_editor_form", clear_on_submit=False):
                ...
                with col_info:
                    st.caption("💡 Use el icono 🗑️ para eliminar filas. Haga clic en una celda para editar.")
        ...
        # Usar datos de session state para cálculos
        ...
        with col_metrics[3]:
            st.metric("Recuperados", f"{df_v_calc['Recuperados'].iloc[-1]:,}")
        with col_metrics[4]:
            st.metric("Fallecidos", f"{df_v_calc['Fallecidos'].sum():,}")
        ...
        if len(df_v_calc) > 3:
            with st.spinner("⏳ Ejecutando modelo..."):
                ...
                if model_type == "SEIR + Monte Carlo":
                    ...
                    fig_proj.add_trace(go.Scatter(
                        x=list(df_v_calc['Dia']) + futuro_dias,
                        y=list(df_v_calc['Activos']) + p_activos_mean.tolist() if len(df_v_calc) < len(futuro_dias) else list(df_v_calc['Activos']) + p_activos_mean.tolist(),
                        name="Activos",
                    ...
                    if st.button("🔮 EJECUTAR ARIMA", use_container_width=True):
                        try:
                            arima_result = calculate_arima_forecast(df_v_calc['Nuevos'].values, periods=15, order=(1, 1, 1))
