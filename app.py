...
if not st.session_state.get("auth", False):
    ...
    # ==========================================
        ...
        # Verificar que df_v_calc no sea None y tenga datos
        ...
        with col_metrics[3]:
            recovered_val = df_v_calc['Recuperados'].iloc[-1] if df_v_calc is not None and len(df_v_calc) > 0 else 0
            st.metric("Recuperados", f"{recovered_val:,}")
        with col_metrics[4]:
            deaths_val = df_v_calc['Fallecidos'].sum() if df_v_calc is not None and len(df_v_calc) > 0 else 0
        ...
        if df_v_calc is not None and len(df_v_calc) > 3:
        ...
        # Verificar que los datos existen antes de generar PDF
        can_generate_pdf = st.session_state.df_v is not None and len(st.session_state.df_v) > 0

        if can_generate_pdf:
            # Recalcular métricas por si hay cambios
            r0_pdf = beta / gamma
            ...
            st.info("📋 Agregue datos en la tabla para generar el reporte PDF")
